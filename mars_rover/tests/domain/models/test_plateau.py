...
def test_inside_true_for_valid_cells():
    ...
    assert board.inside(5, 5)  # inclusive upper bound

def test_inside_false_for_out_of_bounds():
    board = Plateau(5, 5, obstacles=set())
    assert not board.inside(-1, 0)
    assert not board.inside(0, 6)  # was 5; now 6 to reflect inclusive bound
    assert not board.inside(6, 0)  # was 5; now 6 to reflect inclusive bound
